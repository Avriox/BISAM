...
// ADDED BY JAKOB
// Helper function to split matrix (X) with proper block structure
std::vector<double*> splitMatrix(double* arr, int total_rows, int total_cols, int sub_rows, int sub_cols) {
    std::vector<double*> result;
    int n_splits = total_cols / sub_cols;  // Number of splits based on columns

//    Rprintf("\nMatrix Splitting Debug:");
//    Rprintf("\n  Total dimensions: %dx%d", total_rows, total_cols);
//    Rprintf("\n  Sub dimensions: %dx%d", sub_rows, sub_cols);
//    Rprintf("\n  Number of splits: %d", n_splits);

    for (int i = 0; i < n_splits; ++i) {
//        Rprintf("\n\nProcessing split %d:", i);

        // Allocate memory for this split
        double* block = new double[sub_rows * sub_cols];

        // Calculate starting positions
        int start_col = i * sub_cols;
        int start_row = i * sub_rows;  // Now correctly offsetting the starting row for each split
//        Rprintf("\n  Start column: %d", start_col);
//        Rprintf("\n  Start row: %d", start_row);

        // Copy the block
        for (int col = 0; col < sub_cols; ++col) {
            for (int row = 0; row < sub_rows; ++row) {
                // Source index calculation in column-major order
                int src_idx = (start_row + row) + (start_col + col) * total_rows;
                // Destination index in column-major order
                int dest_idx = row + col * sub_rows;
                block[dest_idx] = arr[src_idx];
            }
        }

        // Print the resulting block
//        Rprintf("\n\n  Resulting block (%dx%d):", sub_rows, sub_cols);
//        for(int row = 0; row < sub_rows; row++) {
//            Rprintf("\n  [");
//            for(int col = 0; col < sub_cols; col++) {
//                Rprintf(" %.4f", block[row + col * sub_rows]);
//                if (col < sub_cols-1) Rprintf(",");
//            }
//            Rprintf(" ]");
//        }
...
// Helper function to split vector (y)
std::vector<double*> splitVector(double* arr, int total_length, int sub_size) {
    std::vector<double*> result;
    int n_splits = total_length / sub_size;

    for (int i = 0; i < n_splits; ++i) {
        double* block = new double[sub_size];
        int start = i * sub_size;

        // Simple copy of consecutive elements
        std::copy(arr + start, arr + start + sub_size, block);
        result.push_back(block);
    }
    return result;
}

// Helper function to split integer vector (for deltaini or other integer vectors)
std::vector<int*> splitIntVector(int* arr, int total_length, int sub_size) {
    std::vector<int*> result;
    int n_splits = total_length / sub_size;

    for (int i = 0; i < n_splits; ++i) {
        int* block = new int[sub_size];
        int start = i * sub_size;
...
// Helper function to print matrix (array with dimensions)
void printMatrix(const char* label, double* arr, int rows, int cols) {
    Rprintf("\n%s (%dx%d):\n", label, rows, cols);
    for(int i = 0; i < rows; i++) {
        Rprintf("  [");
        for(int j = 0; j < cols; j++) {
            Rprintf(" %8.4f", arr[i + j * rows]);
            if (j < cols-1) Rprintf(",");
        }
        Rprintf(" ]\n");
    }
...
void printVector(const char* label, double* arr, int length) {
    Rprintf("\n%s (length=%d):\n  [", label, length);
    for(int i = 0; i < length; i++) {
        Rprintf(" %8.4f", arr[i]);
        if (i < length-1) Rprintf(",");
    }
    Rprintf(" ]\n");
}

// Helper function to print integer vector
void printIntVector(const char* label, int* arr, int length) {
    Rprintf("\n%s (length=%d):\n  [", label, length);
    for(int i = 0; i < length; i++) {
        Rprintf(" %d", arr[i]);
        if (i < length-1) Rprintf(",");
...
void printLogicalVector(const char* label, int* arr, int length) {
    Rprintf("\n%s (length=%d):\n  [", label, length);
    for(int i = 0; i < length; i++) {
        Rprintf(" %s", arr[i] ? "TRUE" : "FALSE");
        if (i < length-1) Rprintf(",");
    }
    Rprintf(" ]\n");
}
...
// Split version
SEXP modelSelectionGibbsCISplit(SEXP SpostModeini, SEXP SpostModeiniProb, SEXP Sknownphi, SEXP Sfamily, SEXP SpriorCoef, SEXP SpriorGroup, SEXP Sniter, SEXP Sthinning, SEXP Sburnin, SEXP Sndeltaini, SEXP Sdeltaini, SEXP Sincludevars, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Suncens, SEXP Ssumy2, SEXP Ssumy, SEXP Ssumlogyfact, SEXP Sx, SEXP Scolsumsx, SEXP ShasXtX, SEXP SXtX, SEXP SytX, SEXP Smethod, SEXP Sadjoverdisp, SEXP Shesstype, SEXP SoptimMethod, SEXP Soptim_maxit, SEXP Sthinit, SEXP Susethinit, SEXP SB, SEXP Salpha, SEXP Slambda, SEXP Sphi, SEXP Stau, SEXP Staugroup, SEXP Staualpha, SEXP Sfixatanhalpha, SEXP Sr, SEXP SpriorDelta, SEXP SprDeltap, SEXP SparprDeltap, SEXP SpriorConstr, SEXP SprConstrp, SEXP SparprConstrp, SEXP Sgroups, SEXP Sngroups, SEXP Snvaringroup, SEXP Sconstraints, SEXP Sinvconstraints, SEXP Sverbose, SEXP Ssplit_x, SEXP Sn_timeperiods, SEXP Sn_observations) {
    int *verbose = INTEGER(Sverbose);

    if (*verbose == 1) {
        Rprintf("\n=== Starting Split Method ===\n");
    }

    int n_timeperiods = INTEGER(Sn_timeperiods)[0];
    int n_observations = INTEGER(Sn_observations)[0];
    int total_observations = INTEGER(Sn)[0];
    int n_splits = total_observations / n_timeperiods;
    int ndeltaini = INTEGER(Sndeltaini)[0];

    if (*verbose == 1) {
        Rprintf("\nDimensions:");
        Rprintf("\n  n_timeperiods: %d", n_timeperiods);
        Rprintf("\n  n_observations: %d", n_observations);
        Rprintf("\n  total_observations: %d", total_observations);
        Rprintf("\n  n_splits: %d", n_splits);
        Rprintf("\n  Sp[0]: %d", INTEGER(Sp)[0]);
        Rprintf("\n  ndeltaini: %d", ndeltaini);
    }

    // Split the input arrays for X and y
    std::vector<double*> split_x = splitMatrix(REAL(Sx), total_observations, INTEGER(Sp)[0],
                                               n_timeperiods, INTEGER(Sp)[0] / (total_observations / n_timeperiods));
    std::vector<double*> split_y = splitVector(REAL(Sy), total_observations, n_timeperiods);



    // For deltaini, only split if it has elements
    std::vector<int*> split_deltaini;
    if (ndeltaini > 0) {
        split_deltaini = splitIntVector(INTEGER(Sdeltaini), ndeltaini, ndeltaini / n_splits);
    }

    if (*verbose == 1) {
        // Print original data
        printMatrix("Original X matrix", REAL(Sx), total_observations, INTEGER(Sp)[0]);
        printVector("Original y vector", REAL(Sy), total_observations);
        if (ndeltaini > 0) {
            printIntVector("Original deltaini vector", INTEGER(Sdeltaini), ndeltaini);
        } else {
            Rprintf("\nNo deltaini values (empty vector)");
        }

        // Print split data for first two splits
        for(int i = 0; i < std::min(2, n_splits); i++) {
            Rprintf("\n=== Split %d ===", i);
            printMatrix("Split X matrix", split_x[i], n_timeperiods, INTEGER(Sp)[0]/n_observations);
            printVector("Split y vector", split_y[i], n_timeperiods);
            if (ndeltaini > 0) {
                printIntVector("Split deltaini vector", split_deltaini[i], ndeltaini / n_splits);
            }
        }
    }
    ...
    int mcmc2save = floor((INTEGER(Sniter)[0] - INTEGER(Sburnin)[0] + .0)/(INTEGER(Sthinning)[0] + .0));
    int mycols = (INTEGER(Sfamily)[0] != 0) ? INTEGER(Sp)[0] : (2 + INTEGER(Sp)[0]);
    int mycols2 = (INTEGER(Sfamily)[0] != 0) ? mycols : (mycols + 2);
    ...
    int *postSample = INTEGER(VECTOR_ELT(ans, 0));
    double *margpp = REAL(VECTOR_ELT(ans, 1));
    int *postMode = INTEGER(VECTOR_ELT(ans, 2));
    double *postModeProb = REAL(VECTOR_ELT(ans, 3));
    double *postProb = REAL(VECTOR_ELT(ans, 4));

    // Initialize results
    std::fill(postSample, postSample + mcmc2save * mycols, 0);
    std::fill(postMode, postMode + mycols, 0);
    std::fill(margpp, margpp + mycols2, 0.0);
    postModeProb[0] = REAL(SpostModeiniProb)[0];

    // Process each split
    for (int i = 0; i < n_splits; ++i) {
        // Create temporary SEXPs for this split
        SEXP split_Sx = PROTECT(Rf_allocVector(REALSXP, n_timeperiods * INTEGER(Sp)[0]));
        SEXP split_Sy = PROTECT(Rf_allocVector(REALSXP, n_timeperiods));
        SEXP split_Sdeltaini;

        if (ndeltaini > 0) {
            split_Sdeltaini = PROTECT(Rf_allocVector(INTSXP, ndeltaini / n_splits));
            std::copy(split_deltaini[i], split_deltaini[i] + ndeltaini / n_splits,
                      INTEGER(split_Sdeltaini));
        } else {
            split_Sdeltaini = Sdeltaini;  // Use original empty vector
        }

        // Copy data to temporary SEXPs
        std::copy(split_x[i], split_x[i] + n_timeperiods * INTEGER(Sp)[0], REAL(split_Sx));
        std::copy(split_y[i], split_y[i] + n_timeperiods, REAL(split_Sy));

        // Create temporary SEXP for n
        SEXP split_Sn = PROTECT(Rf_allocVector(INTSXP, 1));
        INTEGER(split_Sn)[0] = n_timeperiods;

        // Call original function for this split
        SEXP split_result = modelSelectionGibbsCIOriginal(
                SpostModeini, SpostModeiniProb, Sknownphi, Sfamily, SpriorCoef, SpriorGroup,
                Sniter, Sthinning, Sburnin, Sndeltaini, split_Sdeltaini, Sincludevars,
                split_Sn, Sp, split_Sy, Suncens, Ssumy2, Ssumy, Ssumlogyfact,
                split_Sx, Scolsumsx, ShasXtX, SXtX, SytX, Smethod, Sadjoverdisp,
                Shesstype, SoptimMethod, Soptim_maxit, Sthinit, Susethinit,
                SB, Salpha, Slambda, Sphi, Stau, Staugroup, Staualpha,
                Sfixatanhalpha, Sr, SpriorDelta, SprDeltap, SparprDeltap,
                SpriorConstr, SprConstrp, SparprConstrp, Sgroups, Sngroups,
                Snvaringroup, Sconstraints, Sinvconstraints, Sverbose
        );

        // Merge results
        int start_idx = i * (mcmc2save * mycols / n_splits);
        std::copy(
                INTEGER(VECTOR_ELT(split_result, 0)),
                INTEGER(VECTOR_ELT(split_result, 0)) + mcmc2save * mycols / n_splits,
                postSample + start_idx
        );

        for (int j = 0; j < mycols2; j++) {
            margpp[j] += REAL(VECTOR_ELT(split_result, 1))[j] / n_splits;
        }

        if (REAL(VECTOR_ELT(split_result, 3))[0] > postModeProb[0]) {
            postModeProb[0] = REAL(VECTOR_ELT(split_result, 3))[0];
            std::copy(
                    INTEGER(VECTOR_ELT(split_result, 2)),
                    INTEGER(VECTOR_ELT(split_result, 2)) + mycols,
                    postMode
            );
        }

        std::copy(
                REAL(VECTOR_ELT(split_result, 4)),
                REAL(VECTOR_ELT(split_result, 4)) + mcmc2save / n_splits,
                postProb + i * (mcmc2save / n_splits)
        );

        // Clean up
        // Adjust UNPROTECT count based on whether we created split_Sdeltaini
        UNPROTECT(ndeltaini > 0 ? 4 : 3);  // Unprotect split_Sx, split_Sy, split_Sdeltaini, split_Sn
    }

    // Clean up split arrays
    for (auto ptr : split_x) delete[] ptr;
    for (auto ptr : split_y) delete[] ptr;
    for (auto ptr : split_deltaini) delete[] ptr;

    UNPROTECT(1);  // Unprotect ans
    return ans;
}
...
// [[Rcpp::export]]
SEXP modelSelectionGibbsCIOriginal(SEXP SpostModeini, SEXP SpostModeiniProb, SEXP Sknownphi, SEXP Sfamily, SEXP SpriorCoef, SEXP SpriorGroup, SEXP Sniter, SEXP Sthinning, SEXP Sburnin, SEXP Sndeltaini, SEXP Sdeltaini, SEXP Sincludevars, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Suncens, SEXP Ssumy2, SEXP Ssumy, SEXP Ssumlogyfact, SEXP Sx, SEXP Scolsumsx, SEXP ShasXtX, SEXP SXtX, SEXP SytX, SEXP Smethod, SEXP Sadjoverdisp, SEXP Shesstype, SEXP SoptimMethod, SEXP Soptim_maxit, SEXP Sthinit, SEXP Susethinit, SEXP SB, SEXP Salpha, SEXP Slambda, SEXP Sphi, SEXP Stau, SEXP Staugroup, SEXP Staualpha, SEXP Sfixatanhalpha, SEXP Sr, SEXP SpriorDelta, SEXP SprDeltap, SEXP SparprDeltap, SEXP SpriorConstr, SEXP SprConstrp, SEXP SparprConstrp, SEXP Sgroups, SEXP Sngroups, SEXP Snvaringroup, SEXP Sconstraints, SEXP Sinvconstraints, SEXP Sverbose) {
...
// Main entry function that decides which version to call
// [[Rcpp::export]]
SEXP modelSelectionGibbsCI(SEXP SpostModeini, SEXP SpostModeiniProb, SEXP Sknownphi, SEXP Sfamily, SEXP SpriorCoef, SEXP SpriorGroup, SEXP Sniter, SEXP Sthinning, SEXP Sburnin, SEXP Sndeltaini, SEXP Sdeltaini, SEXP Sincludevars, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Suncens, SEXP Ssumy2, SEXP Ssumy, SEXP Ssumlogyfact, SEXP Sx, SEXP Scolsumsx, SEXP ShasXtX, SEXP SXtX, SEXP SytX, SEXP Smethod, SEXP Sadjoverdisp, SEXP Shesstype, SEXP SoptimMethod, SEXP Soptim_maxit, SEXP Sthinit, SEXP Susethinit, SEXP SB, SEXP Salpha, SEXP Slambda, SEXP Sphi, SEXP Stau, SEXP Staugroup, SEXP Staualpha, SEXP Sfixatanhalpha, SEXP Sr, SEXP SpriorDelta, SEXP SprDeltap, SEXP SparprDeltap, SEXP SpriorConstr, SEXP SprConstrp, SEXP SparprConstrp, SEXP Sgroups, SEXP Sngroups, SEXP Snvaringroup, SEXP Sconstraints, SEXP Sinvconstraints, SEXP Sverbose, SEXP Ssplit_x, SEXP Sn_timeperiods, SEXP Sn_observations) {

    bool split_x = LOGICAL(Ssplit_x)[0];

    if (split_x) {
        return modelSelectionGibbsCISplit(
                SpostModeini, SpostModeiniProb, Sknownphi, Sfamily, SpriorCoef, SpriorGroup,
                Sniter, Sthinning, Sburnin, Sndeltaini, Sdeltaini, Sincludevars,
                Sn, Sp, Sy, Suncens, Ssumy2, Ssumy, Ssumlogyfact,
                Sx, Scolsumsx, ShasXtX, SXtX, SytX, Smethod, Sadjoverdisp,
                Shesstype, SoptimMethod, Soptim_maxit, Sthinit, Susethinit,
                SB, Salpha, Slambda, Sphi, Stau, Staugroup, Staualpha, Sfixatanhalpha, Sr, SpriorDelta, SprDeltap, SparprDeltap,
                SpriorConstr, SprConstrp, SparprConstrp, Sgroups, Sngroups,
                Snvaringroup, Sconstraints, Sinvconstraints, Sverbose,
                Ssplit_x, Sn_timeperiods, Sn_observations
        );
    } else {
        return modelSelectionGibbsCIOriginal(
                SpostModeini, SpostModeiniProb, Sknownphi, Sfamily, SpriorCoef, SpriorGroup,
                Sniter, Sthinning, Sburnin, Sndeltaini, Sdeltaini, Sincludevars,
                Sn, Sp, Sy, Suncens, Ssumy2, Ssumy, Ssumlogyfact,
                Sx, Scolsumsx, ShasXtX, SXtX, SytX, Smethod, Sadjoverdisp,
...
SEXP pmomMarginalKI(SEXP Ssel, SEXP Snsel, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Ssumy2, SEXP SXtX, SEXP SytX, SEXP Sphi, SEXP Stau, SEXP Sr, SEXP Smethod, SEXP SB, SEXP Slogscale, SEXP Sngroups, SEXP Snvaringroup) {
  ...
  set_marginalPars(&pars,&emptyint,INTEGER(Sn),INTEGER(Sn),INTEGER(Sp),REAL(Sy),&emptyint,REAL(Ssumy2),&emptydouble,&emptydouble,&emptydouble,&emptydouble,XtX,REAL(SytX),INTEGER(Smethod),&emptyint,&emptyint,&SoptimMethod,&Soptim_maxit,&usethinit,&emptydouble,INTEGER(SB),&emptydouble,&emptydouble,&knownphi,REAL(Sphi),REAL(Stau),&emptydouble,taualpha,taualpha,INTEGER(Sr),NULL,NULL,NULL,NULL,INTEGER(Slogscale),&offset,NULL,NULL,INTEGER(Sngroups),NULL,INTEGER(Snvaringroup));
  PROTECT(ans = Rf_allocVector(REALSXP, 1));
  rans = REAL(ans);
...
SEXP pimomMarginalKI(SEXP Ssel, SEXP Snsel, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Ssumy2, SEXP SXtX, SEXP SytX, SEXP Sphi, SEXP Stau, SEXP Smethod, SEXP SB, SEXP Slogscale, SEXP Sngroups, SEXP Snvaringroup) {
  ...
  XtX= new crossprodmat(REAL(SXtX),INTEGER(Sn)[0],INTEGER(Sp)[0],true);
  set_marginalPars(&pars,&emptyint,n,n,p,y,&emptyint,sumy2,&emptydouble,&emptydouble,&emptydouble,&emptydouble,XtX,ytX,method,&emptyint,&emptyint,&SoptimMethod,&Soptim_maxit,&usethinit,&emptydouble,B,&emptydouble,&emptydouble,&knownphi,phi,tau,&emptydouble,taualpha,taualpha,&r,NULL,NULL,NULL,NULL,logscale,&offset,NULL,NULL,INTEGER(Sngroups),NULL,INTEGER(Snvaringroup));
  PROTECT(ans = Rf_allocVector(REALSXP, 1));
  rans = REAL(ans);
...
SEXP pimomMarginalUI(SEXP Ssel, SEXP Snsel, SEXP Sn, SEXP Sp, SEXP Sy, SEXP Ssumy2, SEXP Sx, SEXP SXtX, SEXP SytX, SEXP Stau, SEXP Smethod, SEXP SB, SEXP Slogscale, SEXP Salpha, SEXP Slambda, SEXP Sngroups, SEXP Snvaringroup) {
  ...
  XtX= new crossprodmat(REAL(SXtX),INTEGER(Sn)[0],INTEGER(Sp)[0],true);
  set_marginalPars(&pars,&emptyint,n,n,p,y,&emptyint,sumy2,&emptydouble,&emptydouble,x,&emptydouble,XtX,ytX,method,&emptyint,&emptyint,&SoptimMethod,&Soptim_maxit,&usethinit,&emptydouble,B,alpha,lambda,&knownphi,&emptydouble,tau,&emptydouble,taualpha,taualpha,&r,NULL,NULL,NULL,NULL,logscale,&offset,NULL,NULL,INTEGER(Sngroups),NULL,INTEGER(Snvaringroup));
  PROTECT(ans = Rf_allocVector(REALSXP, 1));
  rans = REAL(ans);
