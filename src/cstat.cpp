...
void sumsqbyclus(double *x, int n, int p, int *z, int nclus, bool lowertri, double ***S) {
    ...
    xbar   = dmatrix(1, nclus, 1, p);
    m2     = dmatrix(1, nclus, 1, p);
    ...
    for (j = 1; j <= p; j++) {
        ...
        for (l = j + 1; l <= p; l++) {
            ...
            for (kk = 1; kk <= nclus; kk++) {
                S[kk][j][l] = S[kk][j][l] - ((double) zcount[kk]) * xbar[kk][j] * xbar[kk][l];
            }
        }
    }
    if (lowertri) {
        for (j = 1; j <= p; j++) {
            for (l = 1; l < j; l++) { for (kk = 1; kk <= nclus; kk++) { S[kk][l][j] = S[kk][j][l]; } }
        }
    }
    free_ivector(zcount, 1, nclus);
    free_dmatrix(xbar, 1, nclus, 1, p);
    free_dmatrix(m2, 1, nclus, 1, p);
}
...
void iindexsort(int *x, int *index, int ilo, int ihi, int incr) {
    ...
    */
    ...
    ulo    = ilo;
    uhi    = ihi; // Initialize ends of unpartitioned region
    // While the unpartitioned region is not empty, try to reduce its size.
    while (ulo < uhi) {
        if ((x[index[uhi]] * incr) > (x[index[pivot]] * incr)) {
            // Here, we can reduce the size of the unpartitioned region and try again.
            if ((uhi < ihi) && ((x[index[uhi]] * incr) > (x[index[uhi + 1]] * incr))) sortup = 0;
            // Check if upper subvector is ordered
            uhi--;
            if ((uhi == pivot) && (ulo < pivot)) {
                tempEntry        = index[pivot];
...
double apnorm2(double y, bool logscale) {
    ...
    if (y <= -4.056531) {
        ...
        double a1 = 0.319381530, a2 = -0.356563782, a3 = 1.781477937, a4 = -1.821255978, a5 = 1.330274429, p = 0.2316419
                , t = 1.0 / (1.0 + p * (-y)), t2 = t * t, t3 = t2 * t, t4 = t2 * t2;
...
double dmvnorm0(const double *y, int p, double *cholsinv, double det, int logscale, bool logdet = false) {
    ...
    */
    ...
    for (i = 1; i <= p; i++) {
        ii = (i - 1) * p - (i - 1) * (i - 2) / 2;
        for (j = i, z2[i] = 0; j <= p; j++) { z2[i] += cholsinv[ii + j - i] * y[j]; }
        //z2[i] += cholsinv[j][i] * y[j]; (only for j<=i)
    }
...
void dmvnormmatC(double *ans, const double *y, int n, int p, const double *mu, double **cholsinv, double det,
                 bool transpose, int logscale) {
    int i, j;
...
double complexPrior(int k,
    ...
    //assert((logscale == 0) || (logscale == 1));
    priornorm = log(1.0 - 1.0 / pow((double) p, priorc * ((double) p + 1.0))) -
                log(1.0 - 1.0 / pow((double) p, priorc));
...
void rtmvnormMH(double *ansortho, double *paccept, int n, int p, double *alpha, double **D, double **K, double det,
                double *lower, double *upper, int within) {
    int i, j, naccept;
...
void rtmvnormOutside_Gibbs(double *z, double *Dj, double *alpha, double **D, int p, double *lower, double *upper) {
    ...
    for (j = 1; j <= p; j++) {
        ...
        if (nrestrict > 0) {
            ...
            lmod[k] = 1.0e20;
            //Draw random variate
            rnorm_truncMult(z + j, &lprop, &one, umod + 1, lmod + 1, k, alpha + j, &oned);
            //note: umod is lower bound for inclusion intervals (lmod is upper bound)
...
void rtmvnormProp(double *z, double *propdens, int p, double *alpha, double **D, double *lower, double *upper,
    ...
    if (within == 1) {
        ...
        for (j = 2; j <= p; j++) {
            Dj = 0;
            for (k = 1; k <= (j - 1); k++) Dj += D[j][k] * z[k];
            l[2] = (upper[j] - Dj) / D[j][j];
            u[1] = (lower[j] - Dj) / D[j][j];
...
void rtmvnormProd(double *ans, int n, int p, double *mu, double **Sinv, int k, double lower, double upper,
                  int is_low_trunc, int is_up_trunc, int burnin) {
    if (is_low_trunc == 1 && is_up_trunc == 0) {
        ...
        y     = dvector(1, p);
        S     = dmatrix(1, p, 1, p);
        cholS = dmatrix(1, p, 1, p);
        inv_posdef(Sinv, p, S, &posdef);
        choldc(S, p, cholS, &posdef);
        rmvnormC(y, p, mu, cholS);
        free_dvector(y, 1, p);
        free_dmatrix(S, 1, p, 1, p);
        free_dmatrix(cholS, 1, p, 1, p);
    }
}

// [[Rcpp::export]]
SEXP rtmvnormProdCI(SEXP n, SEXP mu, SEXP Sigma, SEXP k, SEXP lower, SEXP upper, SEXP is_low_trunc, SEXP is_up_trunc,
                    SEXP burnin) {
    bool posdef;
    ...
    rtmvnormProd(REAL(ans),
                 ...
                 REAL(lower)[0],
                 REAL(upper)[0],
                 INTEGER(is_low_trunc)[0],
                 INTEGER(is_up_trunc)[0],
                 INTEGER(burnin)[0]);
...
void rtmvnormProd_lowup(double *ans, int n, int p, double *mu, double **Sinv, int k, double lower, double upper,
    ...
    rtrunc  = dvector(1, 2);
    for (i = 1; i <= p; i++) {
        condvar[i] = 1 / Sinv[i][i];
        condsd[i]  = sqrt(condvar[i]);
    }
    //Initialize
    l = pow(lower, 1.0 / (k * p + .0));
    u = pow(upper, 1.0 / (k * p + .0));
    for (i = 1; i <= p; i++) {
        if (mu[i] >= l && mu[i] <= u) xcur[i] = mu[i];
        else if (mu[i] < l) xcur[i] = l + .1 * (u - l);
        else if (mu[i] > u) xcur[i] = u - .1 * (u - l);
    }
    //Burn-in
    l = pow(lower, 1.0 / (k + .0));
    u = pow(upper, 1.0 / (k + .0));
    ...
    free_dvector(condvar, 1, p);
    free_dvector(condsd, 1, p);
    free_dvector(xcur, 1, p);
    free_dvector(ltrunc, 1, 2);
    free_dvector(rtrunc, 1, 2);
}
...
void rtmvnormProd_low(double *ans, int n, int p, double *mu, double **Sinv, int k, double lower, int burnin) {
    ...
    for (i = 1; i <= p; i++) {
        condvar[i] = 1 / Sinv[i][i];
        condsd[i]  = sqrt(condvar[i]);
        if (mu[i] > l) xcur[i] = mu[i];
        else xcur[i]           = l + .1 * condsd[i];
    }
    //Burn-in
    l = pow(lower, 1.0 / (k + .0));
...
void rtmvnormProd_up(double *ans, int n, int p, double *mu, double **Sinv, int k, double upper, int burnin) {
    ...
    for (i = 1; i <= p; i++) {
        condvar[i] = 1 / Sinv[i][i];
        condsd[i]  = sqrt(condvar[i]);
        if (mu[i] < u) xcur[i] = mu[i];
        else if (mu[i] >= u) xcur[i] = u - .1 * condsd[i];
    }
    //Burn-in
    u = pow(upper, 1.0 / (k + .0));
...
double qtC(double p,
    ...
    if (fabs(ndf - 2.0) < eps) { /* df ~= 2 */
        ...
        if (y > 0.05 + a) { /* Asymptotic inverse expansion about normal */
            ...
            y = ((1.0 / (((ndf + 6.0) / (ndf * y) - 0.089 * d - 0.822) * (ndf + 2.0) * 3.0) + 0.5 / (ndf + 4.0)) * y -
...
void dimomiggrad(double *ans, int *n, double *th, double *logphi, double *tau, double *alpha, double *lambda) {
    ...
    if (p > 0) { //th is non-empty
        ...
        ans[*n] = 0.5 * p - 0.5 * (*alpha) - 1.0 + 0.5 * (*lambda) * exp(-(*logphi)) - exp(*logphi) * (*tau) *
                  suminvth2;
    } else { //th is empty
        ans[1] = -0.5 * (*alpha) - 1.0 + 0.5 * (*lambda) * exp(-(*logphi));
    }
...
void demomgrad(double *ans, int *n, double *th, double *logphi, double *tau) {
    int i;
    for (i = 1; i <= (*n); i++) {
        ans[i] = 2.0 * (*tau) * exp(*logphi) / pow(th[i], 3.0) - th[i] * exp(-(*logphi)) / (*tau);
    }
}
...
void demomiggrad(double *ans, int *n, double *th, double *logphi, double *tau, double *alpha, double *lambda) {
    ...
    if (p > 0) { //th is non-empty
        ...
        ans[*n] = -0.5 * p - 0.5 * (*alpha) - 1.0 + 0.5 * (sumth2 / (*tau) + (*lambda)) * exp(-(*logphi)) - exp(*logphi)
                  * (*tau) * suminvth2;
    } else { //th is empty
        ans[1] = -0.5 * (*alpha) - 1.0 + 0.5 * (*lambda) * exp(-(*logphi));
    }
...
// [[Rcpp::export]]
SEXP rnlpPostCI_lm(SEXP niter, SEXP burnin, SEXP thinning, SEXP y, SEXP x, SEXP p, SEXP r, SEXP tau, SEXP a_phi,
                   SEXP b_phi, SEXP prior) {
...
void rnlpPost_lm(double *ans, int niter, int burnin, int thinning, double *y, double *x, int n, int p, int r,
                 double tau, double a_phi, double b_phi, int prior) {
    bool posdef;
...
void rnlp_Gibbs(double *th, int p, double *m, double **cholS, double **K, double *tau, double *phi, int r, int prior) {
    ...
    if (prior == 0) {
        for (i = 1; i <= p; i++) {
            ...
            if (r == 1) { upper[i] = sqrt(l[i] * (*tau) * (*phi)); } else {
...
void rnlp_Gibbs_multiple(double *th, double *thini, int p, double *m, double **cholS, double **K, double *tau, int r,
                         int prior, int niter, int burnin, int thinning) {
    int i, isave, nsave, b;
