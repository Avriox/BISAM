...
//
//void covxvec(double *x, int n, int ncol,
//             double **S); //sample covariance. x is a matrix arranged by columns (0-indexed). Answer stored in S[1][1],...,S[p][p]
//void sumsq(double *x, int n, int p, bool lowertri, double **S); //sum of squares and cross-products, that is n*cov(x)
//void sumsqbyclus(double *x, int n, int p, int *z, int nclus, bool lowertri,
//                 double ***S); //sum of squares within clusters indicated by z
//void sum_crossprod(double *x, int n, int p, bool lowertri, double *sumx,
//                   double **crossprodx); //colSums(x) and cross-prod X'X
//void crossprod2sumsq(double **crossprodx, double *xsum, int n, int p, double **S, double *xbar,
//                     bool lowertri); //convert sums into means, cross-products into sums of squares
...
//void nn_bayes(double *mpo, double **Spo, double **Spo_inv, int p, double r1, double *mpr, double **Spr_inv, double r2,
//              double *y, double **Slik_inv);  //Posterior of multiv normal mean with normal prior
//void nn_bayes_rand(double *theta, int p, double r1, double **Spr_inv, double *mpr, double r2, double **Slik_inv,
//                   double *y); //Single draw from posterior of multiv normal mean with normal prior
//double nn_integral(const double *x, const double *rx, double **Vxinv, const double *detVx, const double *mpr,
//                   const double *rpr, double **Vprinv, const double *detVpr, const int *p,
//                   const int *logscale); //Normal-Normal integral (useful to compute Bayes factors etc.)
//
//void lm(double *b, double **XtX, double **invXtX, double *Xty, double *s, double *ypred, const double *y, double **X,
//        const int *n, const int *p, const int *useXtX); //classical multiple linear regression
//void
//lmbayes(double *bpost, double *spost, double *b, double **Vb, double *a_s, double *b_s, double **XtX, double **invXtX,
//        double *Xty, int *B, double *y, double **X, int *n, int *p, int *useXtX, double *mpr, double **Spr_inv,
//        double *tauprior, double *nu0, double *s0); //Bayesian multiple linear regression
//void lmbayes_knownvar(double *bpost, double *b, double **Vb, double **XtX, double **invXtX, double *Xty, double *sigma,
//                      int *B, double *y, double **X, int *n, int *p, int *useXtX, double *mpr, double **Spr_inv,
//                      double *tauprior); //same as lmbayes with known variance sigma^2
...
//double ***darray3(int ini1, int fi1, int ini2, int fi2, int ini3,
//                  int fi3);   //allocate 3-way double array [ini1..fi1][ini2..fi2][ini3..fi3]
int *ivector(int, int);
...
//void mahaldist(double *x, int n, int p, double **cholA, bool squared,
//               double *ans); //Squared Mahalanobis distances (x[i,] - x[j,])' [ cholA %*% t(cholA) ] (x[i,] - x[j,])
//
...
//void rA_plus_sB(double r, double **A, double s, double **B, double **C, int rowini, int rowfi, int colini,
//                int colfi); //matrix*scalar + matrix*scalar
//void rAx_plus_sBy(double r, double **A, const double *x, double s, double **B, const double *y, double *z, int rowini,
//                  int rowfi, int colini, int colfi); //scalar*matrix*vector + scalar*matrix*vector
...
//void Avecx(const double *A, const double *x, double *z, int rowini, int rowfi, int colini,
//           int colfi); //same but A is in vector format
//void Aselvecx(const double *A, const double *x, double *z, int rowini, int rowfi, int *sel,
//              int *nsel); //Returns z=A[,sel] %*% x
//void Atselvecx(const double *A, const double *x, double *z, int rowini, int rowfi, int *sel,
//               int *nsel); //Returns z=t(A[,sel]) %*% x
//void Atvecx(const double *A, const double *x, double *z, int rowini, int rowfi, int colini,
//            int colfi); //same for A' (row/col indexes refer to A')
//double xtAy(const double *x, double **A, const double *y, int ini, int fi); //t(vector)*matrix*vector
//
//double quadratic_xtAx(const double *x, double **A, int ini,
//                      int fi); //t(vector)*matrix*vector for quadratic forms (A symmetric)
//double quadratic_xseltAselxsel(const double *x, crossprodmat *A, const int *ncol, const int *nsel,
...
//void AtB(double **A, int rowiniA, int rowfiA, int coliniA, int colfiA, double **B, int rowiniB, int rowfiB, int coliniB,
//         int colfiB, double **C); //t(matrix)*matrix, stored in C
//void AB(double **A, int rowiniA, int rowfiA, int coliniA, int colfiA, double **B, int rowiniB, int rowfiB, int coliniB,
//        int colfiB, double **C); //matrix * matrix, stored in C
//void ABt(double **A, int rowiniA, int rowfiA, int coliniA, int colfiA, double **B, int rowiniB, int rowfiB, int coliniB,
//         int colfiB, double **C); //matrix * t(matrix), stored in C
//void AvectBvec(double *A, int nrowA, int ncolA, double *B, int nrowB, int ncolB,
//               double **C); //same but input as 0-indexed vector
...
//void a_prod_b_sel(const double *a, const double *b, double *c, const int *lengtha, const int *nsel,
//                  const int *sel); //same but only using indexes in sel
...
//
//void
//minvec(const double *x, int ini, int fi, double *xmin, int *minpos); //min of a vector and position at which min occurs
//void
//maxvec(const double *x, int ini, int fi, double *xmax, int *maxpos); //max of a vector and position at which max occurs
//
//void make_posdef(double **a, int n,
//                 double offset = .01); //Make matrix +def via replacing a by a - (lmin+offset) I, where lmin is smallest eigenvalue of a
...
void inv_posdef(double **a, int n, double **aout, bool *posdef, double **chola = NULL,
                double **cholainv = NULL); //Inverse of a symmetric, positive definite matrix
...
//
//void choldcinv_det(arma::mat *Ainv, arma::mat *cholAinv, double *logdet_Ainv,
//                   arma::mat *A); //inverse, Cholesky decomp and determinant
//
...
void dindexsort(double *x, int *index, int ilo, int ihi,
                int incr); //sort a vector of indexes using self-written quicksort routine
...
//
//void
//rbirthdeath(int *index, bool *birth, arma::SpMat<short> *model, double pbirth); //random draw from birth-death proposal
//double dbirthdeath(arma::SpMat<short> *modelnew, arma::SpMat<short> *model, double pbirth,
//                   bool logscale); //pmf of birth-death proposal
...
double dnormC_jvec(const double *y, int n, double m, double s,
                   int logscale); //joint density of y[0]...y[n-1] under Normal(m,s^2), i.e. returns scalar
//double dmvnormC(const double *y, int p, const double *mu, double **cholsinv, double det, bool transpose, int logscale,
//                bool logdet); //density of multivariate Normal
//double dmvnorm0(const double *y, int p, double **cholsinv, double det, bool transpose, int logscale,
//                bool logdet); //same for particular case mean=0
//double dmvnorm0(const double *y, int p, double *cholsinv, double det, int logscale,
//                bool logdet); //same for particular case mean=0, cholsinv given as vector
//void
//dmvnormmatC(double *ans, const double *y, int n, int p, const double *mu, double **cholsinv, double det, bool transpose,
//            int logscale); //same but y is n x p matrix
//void dmvnormmat_transC(double *ans, const double *ty, int n, int p, const double *mu, double **cholsinv, double det,
//                       bool transpose, int logscale); //same but t(y) is provided
...
void rnorm_truncMult(double *y, double *pdfy, int *n, double *ltrunc, double *rtrunc, int ntrunc, double *m,
                     double *s); //n draws univ trunc Normal given multiple truncation points

//void rtmvnorm(double *ans, int n, int p, double *mu, double **Sigma, double *lower, double *upper, int within,
//              int method); //n draws multiv trunc Normal given rectangular constraints
//void rtmvnormMH(double *ansortho, double *paccept, int n, int p, double *alpha, double **D, double **K, double det,
//                double *lower, double *upper, int within); //Indep prop MH
//void rtmvnormOutside(double *ans, int n, int p, double *alpha, double **D, double *lower,
//                     double *upper); //Gibbs sampling, truncation outside interval
void rtmvnormOutside_Gibbs(double *z, double *Dj, double *alpha, double **D, int p, double *lower,
                           double *upper); //Single Gibbs sampling step
//void rtmvnormWithin(double *ans, int n, int p, double *alpha, double **D, double *lower,
//                    double *upper);  //Gibbs sampling, truncation within interval
//void rtmvnormProp(double *z, double *propdens, int p, double *alpha, double **D, double *lower, double *upper,
//                  int within); //proposal for multiv trunc Normal
//
//void
//rtmvnormProd(double *ans, int n, int p, double *mu, double **Sinv, int k, double lower, double upper, int is_low_trunc,
//             int is_up_trunc, int burnin); //multiv trunc Normal given constraint on product
//void rtmvnormProd_lowup(double *ans, int n, int p, double *mu, double **Sinv, int k, double lower, double upper,
//                        int burnin); //lower and upper truncation on product
//void rtmvnormProd_low(double *ans, int n, int p, double *mu, double **Sinv, int k, double lower,
//                      int burnin); //only lower truncation on product
//void rtmvnormProd_up(double *ans, int n, int p, double *mu, double **Sinv, int k, double upper,
//                     int burnin); //only upper truncation on product
//
//// Moments
//double mnorm(double order, double m, double sd); //raw moment of N(m,sd) of order "order"
//
//double mvtexpect(double *mu, double **sigma, int n, int power,
//                 double dof); //mean of prod (x_i)^(2*power) when x_i ~ T_dof(mu,sigma). Set dof=-1 for N(mu,sigma). Written by John Cook
//double
//mvtexpect_vec(double *mu, double *sigma, int n, int power, double dof); //same but input args are 0-indexed vectors
//
//// T Student
//double dtC(double y, double mu, double s, int nu); //density of t with nu df
//double dtmixC(double y, const double *mu, const double *s, const double *probs, int nu, int ncomp,
...
//
//void rwishartC(double **ans, int df, double **cholS, int p,
//               bool returnChol);  //draw from Wishart(df,S), where S= t(cholS) %*% cholS
//
//// Gamma & Inverse gamma
...
//void dmomiggrad(double *ans, int *n, double *th, double *logphi, double *tau, double *alpha,
//                double *lambda); //Grad log-pMOM + log-IG wrt (th,logphi)
//void dmomighess(double **ans, int *n, double *th, double *logphi, double *tau, double *alpha,
//                double *lambda); //Hess log-pMOM + log-IG wrt (th,logphi)
//
...
//void dimomiggrad(double *ans, int *n, double *th, double *logphi, double *tau, double *alpha,
//                 double *lambda); //Grad log-piMOM + log-IG wrt (th,logphi)
//void dimomighess(double **ans, int *n, double *th, double *logphi, double *tau, double *alpha,
//                 double *lambda);//Hess log-piMOM + log-IG wrt (th,logphi)
//
...
//void demomiggrad(double *ans, int *n, double *th, double *logphi, double *tau, double *alpha,
//                 double *lambda); //Grad log-peMOM + log-IG wrt (th,logphi)
//void demomighess(double **ans, int *n, double *th, double *logphi, double *tau, double *alpha,
//                 double *lambda);//Hess log-peMOM + log-IG wrt (th,logphi)
//

// Posterior sampling under non-local priors
void rnlpPost_lm(double *ans, int niter, int burnin, int thinning, double *y, int *x, int n, int p, int r, double tau,
                 double a_phi, double b_phi, int prior, double *thini,
                 double phiini); //NLP posterior samples under linear model
//void rnlp(double *ans, int niter, int burnin, int thinning, double *m, double *Vvec, int p, int r, double tau,
//          int prior); //NLP samples based on mean & covar
void rnlp_Gibbs(double *th, int p, double *m, double **cholS, double **K, double *tau, double *phi, int r,
                int prior); //single Gibbs update
//void rnlp_Gibbs_multiple(double *th, double *thini, int p, double *m, double **cholS, double **K, double *tau, int r,
//                         int prior, int niter, int burnin, int thinning); //multiple updates
...
double
pen_imom(double *th, double *phi, double *tau, int logscale); //iMOM penalty dimom(th,tau*phi) / dnorm(th,0,tau*phi)
...
//double midpnt(double (*const func)(double), double a, double b,
//              int n); //nth stage refinement of integral of func from a to b (evenly spaced in x)
//double midinf(double (*const func)(double), double aa, double bb,
//              int n); //nth stage refinement of integral of func from aa to bb (evenly spaced in 1/x)
//double qromo(double (*const func)(double), double a, double b,
//             double (*const choose)(double(*const )(double), double, double,
//                                    int)); //Romberg integr on open interval (a,b)
...
//void bspline(double **W, const double *x, const int *nx, const int *degree, const double *knots,
//             const int *nknots); //B-spline basis eval at vector of values x
//void bspline_vec(double *W, const double *x, const int *nx, const int *degree, const double *knots,
//                 const int *nknots); //same as bspline but returns a vector, so that it can be called from R
//void mspline(double **W, const double *x, const int *nx, const int *degree, const double *knots,
//             const int *nknots); //M-spline basis eval at vector of values x
//void mspline_vec(double *W, const double *x, const int *nx, const int *degree, const double *knots,
//                 const int *nknots); //same as mspline but returns a vector, so that it can be called from R
...
double univmin(double ax, double bx, double cx, double (*const f)(double), double tol, double *xmin,
               int itmax); //univariate minim
//double dunivmin(double ax, double bx, double cx, double (*const f)(double), double (*const df)(double), double tol,
//                double *xmin, int itmax);
//
void minimize(double th[], double **dirini, int n, double ftol, int *iter, double *fret, double (*const f)(double []),
              int itmax);//multivar minim
void dirmin(double p[], double xi[], int n, double *fret, double (*const func)(double []), int itmax,
            double dirminEPS); //minim in 1 direction
void mnbrak(double *ax, double *bx, double *cx, double *fa, double *fb, double *fc,
            double (*const func)(double)); //find bracketing triplets
